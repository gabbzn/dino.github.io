...
(function ($, undefined) {
    ...
    switch (type) {
      ...
      default:
        ...
        return _FormatInput(
          $(
            "<input>",
            $.extend(true, {}, definition.htmlAttributes || {}, {
              type: type,
              maxlength: definition.maxlength,
              placeholder: definition.title,
  ...
  function _FormatInput(input, params) {
    if (params.autotrim || params.autotrim === undefined)
      input.on("blur", _Input_Blur);
    return input
  ...
  function _Window_KeyDown(event) {
    ...
    if (button) {
      ...
      if (
        event.which === 13 &&
        messagebox.find(".messagebox_content_input_textarea:focus").length
      )
        return; // Allows newline in textarea
